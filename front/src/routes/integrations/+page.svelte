
<section>
    <h4>Api's Externas</h4>
    <h2>Domingo Morales Calderon</h2>
            <ul>
                <li class ="li_mod">
                    <a href="/stats-volleyball/###">/evolution</a> : Uso textual de API SOS G13.
                </li>
                <li class ="li_mod">
                    <a href="/stats-volleyball/###">/spotify</a> : Uso textual a través de OAUTH de la API de Spotify. Muestra tus playlists.
                </li>
                <li class ="li_mod">
                    <a href="/stats-volleyball/###">/github</a> : Uso textual a través de OAUTH de la API de GitHub. Muestra información de tu usuario.
                </li>
                <li class ="li_mod">
                    <a href="/stats-volleyball/###">/graph_serie</a> - Uso con widget API de rapid api para ver info de serie.
                </li>
            </ul>
    <h2>Pablo Ruiz Rivas</h2>
            <ul>
                <li class ="li_mod">
                    <a href="/stats-rugby/baseball">/Baseball</a> - Po la de baseball.
                </li>
                <li class ="li_mod">
                    <a href="/stats-rugby/###">/graph_coins</a> - Uso de una api externa con un widget(coins).
                </li>
                <li class ="li_mod">
                    <a href="/stats-rugby/###">/graph_nba</a> - Uso de una api externa con un widget(NBA).
                </li>
                <li class ="li_mod">
                    <a href="/stats-rugby/###">/graph_proxy</a> - Uso de una api de sos(G22) a través de un proxy propio.
                </li>
            </ul>
    <h2>Pablo Suarez Sarmiento</h2>
            <ul>
                <li class ="li_mod">
                    <a href="/stats-football/###">/Twitch API</a> - Uso de una api de twitch para ver todas las canciones de una playlist mediante su id(Oauth).
                </li>
                <li class ="li_mod">
                    <a href="/stats-football/###">/graph_coins</a> - Uso de una api externa con un widget(coins).
                </li>
                <li class ="li_mod">
                    <a href="/stats-football/###">/graph_nba</a> - Uso de una api externa con un widget(NBA).
                </li>
                <li class ="li_mod">
                    <a href="/stats-football/###">/graph_proxy</a> - Uso de una api de sos(G22) a través de un proxy propio.
                </li>
            </ul>

</section>
